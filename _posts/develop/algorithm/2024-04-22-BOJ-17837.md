...
- 1000회까지의 반복을 수행하는 메인 simul() 함수 구현
  - 각각의 말을 0번 인덱스부터 마지막 인덱스까지 움직이는 반복문을 구현
  - 이동시키는 함수 move()와 종료조건을 체크하는 isOver() 함수 구현
- 맵의 상태에 따라서 move() 로직을 차근차근 구현하기
  - 파랑이거나 체스판을 벗어났을 때
    - bitflip을 이용하여 이동 방향 반대로 만들기 {0, 1, 2, 3}
  - 빨강
    - 해당 인덱스부터 반전 삽입
    - reverse 함수를 이용해 해당 위치부터 끝까지 반전시키기 reverse(pos_iter, end_iter);
...
- 구조체를 활용해서 3가지 변수를 담아놓고 상태값으로 활용 가능